...
const STARTUP_SPLASH_AUTO_CLOSE_MS = 850;
...
function initStartupSplash(){
  ...
  // Auto close shortly after first paint
  setTimeout(close, STARTUP_SPLASH_AUTO_CLOSE_MS);
  // Hard failsafe
...
function renderFileList(){
    ...
    list.forEach(f=>{
      ...
      tr.innerHTML = `
        ...
        <td><div class="actions-row">
          ...
          <button class="mini-btn btn-map" onclick="previewOnMap('${f.path}','${name}')" title="Χάρτης: προεπισκόπηση (zoom) χωρίς αλλαγή On/Off"><span class="ico-map">🗺</span> Map</button>
  ...
  addCategory("boundaries", "Διοικητικά Όρια", DATA_GROUPS.boundaries, "🏳️", "#0f0f0f");
