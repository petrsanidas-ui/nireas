...
function renderFileList(){
  ...
  const addCategory = (catKey, title, list, icon) => {
    ...
    const rows = (list || []).filter(f=>{
      ...
      // Optional AOI filter
      // If AOI is active AND we have an expanded AOI set:
      // - show layers whose municipality_ids intersect AOI
      // - hide layers with missing municipality_ids (strict filtering), so you immediately see data gaps
      if(useAOI && expandedSet.size){
        ...
        // Strict: hide layers without municipality_ids when AOI filtering is enabled
        return false;
      }
      return true;
    });
